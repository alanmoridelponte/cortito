...
@if(! $unlocked)
    <div class="mx-auto max-w-md space-y-4">
        <div class="rounded-2xl border border-gray-200 bg-white p-8 text-center shadow-sm">
            ...
            <h2 class="text-xl font-bold text-gray-900">Anotador protegido</h2>
            <p class="mt-2 text-sm text-gray-500">Ingresa la contraseña para ver el contenido.</p>
            ...
            <form method="POST" action="{{ route('snippets.show.password', $snippet->alias) }}" class="mt-6 space-y-3">
                ...
                <input
                    ...
                    required
                    autofocus
                    class="w-full rounded-lg border border-gray-300 px-4 py-3 text-sm focus:border-indigo-500 focus:ring-2 focus:ring-indigo-500/20">
                @error('password')
                    <p class="text-xs text-red-500">{{ $message }}</p>
                @enderror
                <button type="submit" class="w-full rounded-lg bg-indigo-600 px-4 py-3 text-sm font-medium text-white hover:bg-indigo-700 transition-colors cursor-pointer">
                    Desbloquear
                </button>
            </form>
        </div>
...
@else
    <div class="space-y-6" x-data="{ showDeleteConfirm: false }">
        ...
        <nav class="flex items-center gap-1.5 text-sm text-gray-400">
            <a href="{{ route('home') }}" class="text-gray-500 hover:text-indigo-600 transition-colors">Cortitos</a>
            <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/>
            </svg>
            <span class="font-mono text-gray-600">{{ $snippet->alias }}</span>
        </nav>
        ...
        <div class="flex items-start justify-between gap-4">
            ...
            @endphp
            <span class="inline-flex items-center rounded-full px-2.5 py-0.5 text-xs font-medium {{ $typeColors[$snippet->content_type] ?? 'bg-gray-100 text-gray-700' }}">
                        {{ $typeLabels[$snippet->content_type] ?? ucfirst($snippet->content_type) }}
                    </span>
                    @if($snippet->language)
                ...
                <div x-data="{ copied: false, canShare: typeof navigator !== 'undefined' && !!navigator.share }" class="mt-2 flex flex-wrap items-center gap-3 text-xs text-gray-400">
                    ...
                    <button
                        ...
                        "
                        class="inline-flex items-center gap-1 rounded-md px-2 py-1 text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 transition-colors cursor-pointer">
                        <svg class="h-3.5 w-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
                    ...
                    <button
                        ...
                        :class="copied ? 'text-green-600' : 'text-gray-500 hover:text-indigo-600 hover:bg-indigo-50'">
            ...
            @if($snippet->canBeEditedBy(request()))
                <div class="flex items-center gap-2 shrink-0">
                    <button type="button"
                            ...
                            class="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 bg-white px-3.5 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:border-gray-300 transition-colors cursor-pointer">
                    ...
                    <button
                        ...
                        class="inline-flex items-center gap-1.5 rounded-lg border border-red-200 bg-white px-3.5 py-2 text-sm text-red-600 hover:bg-red-50 hover:border-red-300 transition-colors cursor-pointer">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="2">
        ...
        <div class="overflow-hidden rounded-xl border border-gray-200 bg-white shadow-sm">
            @if($snippet->content_type === 'url')
                <div class="p-6 text-center">
                    <p class="text-sm text-gray-500 mb-2">Redirigiendo a:</p>
                    <a href="{{ $snippet->content }}" class="text-indigo-600 hover:text-indigo-800 font-medium break-all">{{ $snippet->content }}</a>
                </div>
            @else
                <pre class="whitespace-pre-wrap p-6 text-sm leading-relaxed text-gray-800">{{ $snippet->content }}</pre>
            @endif
        </div>
        ...
        <div x-show="showDeleteConfirm" x-cloak
            ...
            <div class="w-full max-w-sm rounded-xl bg-white p-6 shadow-2xl" @click.away="showDeleteConfirm = false"
                ...
                <h3 class="mb-1 mt-3 text-lg font-bold text-gray-900">Eliminar cortito</h3>
                <p class="mb-5 text-sm text-gray-500">Se eliminará permanentemente <strong class="font-mono text-gray-700">{{ $snippet->alias }}</strong>. Esta accion no se puede deshacer.</p>

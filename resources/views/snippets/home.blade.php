...
@section('header-actions')
    <button
        ...
        <span class="hidden sm:inline">Armá un cortito</span>
        ...
        <div class="mb-8">
            <div class="flex flex-col gap-6 sm:flex-row sm:items-end sm:justify-between">
                <div>
                    ...
                    <p class="mt-1.5 text-base text-graphite">Links y notas que se hacen humo.</p>
                </div>
    ...
    @if(isset($anonymousLimit) && $anonymousCount >= $anonymousLimit)
        <div class="mb-6 rounded-xl border border-amber/20 bg-amber-light p-4 text-sm text-amber">
            <div class="flex items-center gap-2.5">
                ...
                Llegaste al límite de {{ $anonymousLimit }} cortitos gratis. Borrá alguno o esperá que se venzan para crear más.</div>
        </div>
    @endif
    ...
    @if($snippets->isEmpty())
        <div class="flex flex-col items-center justify-center rounded-2xl border-2 border-dashed border-border-warm bg-warm-white py-24">
            <div class="mb-5 flex h-16 w-16 items-center justify-center rounded-2xl bg-celeste-light">
                <svg class="h-8 w-8 text-celeste-text" fill="none" viewBox="0 0 24 24" stroke="currentColor" stroke-width="1.5">
                    <path stroke-linecap="round" stroke-linejoin="round" d="M13.5 16.875h3.375m0 0h3.375m-3.375 0V13.5m0 3.375v3.375M6 10.5h2.25a2.25 2.25 0 002.25-2.25V6a2.25 2.25 0 00-2.25-2.25H6A2.25 2.25 0 003.75 6v2.25A2.25 2.25 0 006 10.5zm0 9.75h2.25A2.25 2.25 0 0010.5 18v-2.25a2.25 2.25 0 00-2.25-2.25H6a2.25 2.25 0 00-2.25 2.25V18A2.25 2.25 0 006 20.25zm9.75-9.75H18a2.25 2.25 0 002.25-2.25V6A2.25 2.25 0 0018 3.75h-2.25A2.25 2.25 0 0013.5 6v2.25a2.25 2.25 0 002.25 2.25z"/>
                </svg>
            </div>
            <h3 class="mb-1.5 font-display text-lg font-bold text-ink">Todavía no tenés ningún cortito</h3>
            <p class="mb-6 text-sm text-graphite">Mandá tu primer link o nota y empezá a compartir al toque.</p>
            <button
                type="button"
                onclick="window.dispatchEvent(new CustomEvent('open-create-modal'))"
...
function snippetDashboard() {
    return {
        ...
        filters: [
            ...
            { value: 'text', label: 'Textos', count: 0 },
            { value: 'url', label: 'Enlaces', count: 0 },
        ],
